...
sudo --preserve-env=DEBIAN_FRONTEND curl -L https://github.com/docker/compose/releases/download/1.29.2/docker-compose-$(uname -s)-$(uname -m) -o /usr/local/bin/docker-compose
sudo --preserve-env=DEBIAN_FRONTEND chmod +x /usr/local/bin/docker-compose
sudo --preserve-env=DEBIAN_FRONTEND ln -s /usr/local/bin/docker-compose /usr/bin/docker-compose

# INSTALL DEPENDENCIES AND START RUNNER
curl -s https://raw.githubusercontent.com/actions/runner/main/src/Misc/layoutbin/installdependencies.sh | sudo --preserve-env=DEBIAN_FRONTEND --preserve-env=RUNNER_CFG_PAT bash
